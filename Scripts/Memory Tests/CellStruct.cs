...
public class CellStruct : MonoBehaviour
{
    ...
    private IEnumerator Start()
    {
        ...
        if (!done)
        {
            ...
            if (doubleDungeon)
            {
                Debug.Log("doubling dungeon");
                yield return StartCoroutine("DoubleDungeon");
    ...
    IEnumerator RecursiveBacktracking()
    {
    ...
    IEnumerator PlaceRooms()
    {
        for (int i = 0; i < noOfRooms; i++)
        {
            ...
            for (int x = (int)bestPos.x; x < (int)bestPos.x + roomWidth; x++)
            {
                ...
                {
                    Vector2 pos = new Vector2(x, y);
                    //Debug.Log(string.Format("pos {0}, {1}", x, y));
                    CellS c = cells[pos];
